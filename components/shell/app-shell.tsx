...
import { sair } from "@/server/actions/auth/entrar-com-senha"
import { useTheme } from "@/hooks/use-theme"
import { cn } from "@/lib/utils"
import type { PerfilAtual } from "@/server/queries/auth/perfil-atual"

const navItems = [
  { label: "Home", href: "/home", icon: Home, badge: null },
  { label: "Projetos", href: null, icon: FolderKanban, badge: null },
  { label: "Meu Time", href: null, icon: Users, badge: null },
  { label: "Feed Geral", href: null, icon: Newspaper, badge: null },
  { label: "Configurações", href: null, icon: Settings, badge: null },
] as const

const roleLabels: Record<PerfilAtual["roleGlobal"], string> = {
  ...
  return (
    <div className="flex items-center gap-3 font-montserrat text-lg font-semibold text-text-1">
      ...
      <div className="leading-tight">
        ...
        <p className="text-[10px] font-semibold uppercase text-text-3">Risca</p>
      </div>
    </div>
  )
}
...
function NavItem({
  ...
  const content = (
    <>
      ...
      <span className={cn("truncate", compact && "text-[11px]")}>
        {item.label}
      </span>
      {item.badge ? (
...
function UserSummary({ perfil }: { perfil: PerfilAtual }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        <p className="truncate text-sm font-semibold text-text-1">
          {perfil.apelido ?? perfil.nomeCompleto}
        </p>
        <p className="truncate text-xs text-text-3">
          {roleLabels[perfil.roleGlobal]}
          {perfil.times[0] ? ` · ${perfil.times[0].nome}` : ""}
        </p>
      </div>
    </div>
  )
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 flex-col border-r border-border bg-surface-1 md:flex">
        ...
        <div className="border-t border-border p-4">
          <div className="space-y-3 rounded-lg border border-border bg-surface-2 p-3">
            <UserSummary perfil={perfil} />
            <form action={sair}>
              <button
                type="submit"
                className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-md border border-border bg-transparent px-3 text-sm font-semibold text-text-2 hover:bg-surface-3 hover:text-text-1"
              >
      ...
      <div className="flex min-h-screen flex-col md:pl-72">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border bg-surface-1/95 px-4 backdrop-blur md:px-6">
          ...
          <div className="flex items-center gap-2">
            <span className="hidden rounded-md border border-terracota/30 bg-terracota/10 px-2.5 py-1 text-xs font-semibold text-terracota sm:inline-flex">
              {roleLabels[perfil.roleGlobal]}
            </span>
            <button
              type="button"
